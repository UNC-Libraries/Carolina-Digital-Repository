 ...
 * @author bbpennel
 */
public class TransformDepositPremisService extends PremisTransformationService {

    /**
     * Constructor a deposit record batch transformation service
     *
     ...
     */
    public TransformDepositPremisService(Path premisListPath, Path outputPath) {
        super(premisListPath, outputPath);
    }

    @Override
    protected PID makePid(Path premisPath) {
        return PIDs.get(DEPOSIT_RECORD_BASE, extractUUIDFromPath(premisPath));
    }
    ...
    protected AbstractPremisToRdfTransformer makeTransformer(PID pid, Path docPath) {
        ...
        return new DepositRecordPremisToRdfTransformer(pid, premisLogger, docPath);
    }
